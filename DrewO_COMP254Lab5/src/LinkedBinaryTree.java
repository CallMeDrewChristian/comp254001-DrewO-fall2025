...
public class LinkedBinaryTree<E> extends AbstractBinaryTree<E> {
  ...
  //Drew Oro 301337741//
  /*
  Preorder goes Root, Left, then Right
  Given a
   */
  public Position<E> preorderNext(Position<E> p) {
    ...
    Node<E> parent = node.getParent();
    //It first goes to the left node. If not, then checks the right node
    if (leftnode != null) {
      return left(node);
    }
    if (rightnode != null) {
    ...
    //If no children from left or right, we check the parent and find the next node.
    //Since the parent is already found, we should identify the right node
    Node<E> currentNode = node;
    while(parent != null) {
      if (currentNode == parent.getLeft() && parent.getRight() != null) {
