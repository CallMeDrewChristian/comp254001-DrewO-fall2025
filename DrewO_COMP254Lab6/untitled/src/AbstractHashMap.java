...
public abstract class AbstractHashMap<K,V> extends AbstractMap<K,V> {
  private final double maxLoad; //added MaxLoad 301337741
  ...
  public AbstractHashMap(int cap, int p, double maxLoad) { //Added Max Load to make user customize the max load 301337741
    prime = p;
    capacity = cap;
    this.maxLoad = maxLoad; //301337741
  ...
  public AbstractHashMap(int cap, int p) {
    this(cap, p, 0.5);  // Default to 0.5 for backward compatibility
  }
  ...
  public AbstractHashMap(double maxLoadFactor) { //This creates a hash table with capacity of 17, prime factor, and  the MaxLoadFactor
  ...
  public V put(K key, V value) {
    ...
    if (n > maxLoad * capacity) {  // Changed to MaxLoad, rather than keeping it 0.5
      resize(2 * capacity - 1);
    }
    return answer;
  }
